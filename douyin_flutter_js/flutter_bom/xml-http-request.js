...
class _XMLHttpRequestUpload {
  constructor() {
    this.$$clazz$$ = "_XMLHttpRequestUpload";
  }
  addEventListener(event, callback) {}
  removeEventListener() {}
}

export class _XMLHttpRequest {
  constructor(pageId) {
    ...
    this.$_status = 0;
    this.$_statusText = "";
    this.$_readyState = _XMLHttpRequest.UNSENT;
    this.$_header = {
      Accept: "*/*",
    };
    this.$_responseType = "";
    ...
    this.eventCallbacks = {};
    this.upload = new _XMLHttpRequestUpload();
  }
  addEventListener(event, callback) {
  ...
  $_callRequest() {
    if (this.$_timeout) {
      ...
      setTimeout(() => {
        if (!this.$_status && this.$_readyState !== _XMLHttpRequest.DONE) {
          if (this.$_requestTask) this.$_requestTask.abort();
          this.$_callReadyStateChange(_XMLHttpRequest.DONE);
  ...
  $_requestSuccess({ data, statusCode, header }) {
    ...
    this.$_callReadyStateChange(_XMLHttpRequest.HEADERS_RECEIVED);
    if (data) {
      this.$_callReadyStateChange(_XMLHttpRequest.LOADING);
  ...
  open(method, url) {
    ...
    this.$_method = method;
    this.$_url = url;
    this.$_callReadyStateChange(_XMLHttpRequest.OPENED);
  }
  ...
  send(data) {
    ...
    if (this.$_readyState !== _XMLHttpRequest.OPENED) return;
...
_XMLHttpRequest.UNSENT = 0;
_XMLHttpRequest.OPENED = 1;
_XMLHttpRequest.HEADERS_RECEIVED = 2;
_XMLHttpRequest.LOADING = 3;
_XMLHttpRequest.DONE = 4;
